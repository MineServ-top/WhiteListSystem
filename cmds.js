...
function wladd(r, msg){
  ...
  if (!nickname || !msg.member.permissions.has(PermissionsBitField.Flags.Administrator)  || !interaction.member.roles.cache.has(client.db.get(conf.guildId))) return msg.channel.send({content: '**У вас нет прав на выполнение команды, либо вы допустили ошибку!**',})
  else{
